...
parser_text.add_argument(
    ...
    default=0,
    type=int,
    help="KV cache size in GB. If not set, cache is allocated dynamically.",
    dest="cache_size",
)
parser_text.add_argument(
    "--draft_source_model",
...
parser_text.add_argument(
    "--draft_eagle3_mode",
    action="store_true",
    help="Set this flag if you use EAGLE3 draft model for speculative decoding",
    dest="draft_eagle3_mode",
)
parser_text.add_argument(
    "--max_prompt_len",
    required=False,
...
parser_text.add_argument(
    ...
    action="store_true",
    help="Set pipeline to use prompt lookup decoding",
    dest="prompt_lookup_decoding",
)
parser_text.add_argument(
    "--reasoning_parser",
    choices=["qwen3", "gptoss"],
    help="Set the type of the reasoning parser for reasoning content extraction",
...
parser_embeddings_ov = subparsers.add_parser(
    "embeddings_ov",
...
parser_embeddings_ov.add_argument(
    "--skip_normalize",
    default=True,
    action="store_false",
...
parser_embeddings_ov.add_argument(
    "--pooling",
    default="CLS",
    choices=["CLS", "LAST", "MEAN"],
    help="Embeddings pooling mode",
    dest="pooling",
)
parser_embeddings_ov.add_argument(
    "--truncate",
...
parser_rerank_ov = subparsers.add_parser(
    "rerank_ov",
...
parser_text2speech = subparsers.add_parser(
    "text2speech", help="export model for text2speech endpoint"
)
add_common_arguments(parser_text2speech)
parser_text2speech.add_argument(
    "--num_streams",
    default=0,
    type=int,
    help="The number of parallel execution streams to use for the models in the pipeline.",
...
parser_text2speech.add_argument(
    "--vocoder",
    type=str,
    help="The vocoder model to use for text2speech. For example microsoft/speecht5_hifigan",
    dest="vocoder",
)
parser_text2speech.add_argument(
    "--speaker_name", type=str, help="Name of the speaker", dest="speaker_name"
)
parser_text2speech.add_argument(
    "--speaker_path",
    type=str,
    help="Path to the speaker.bin file.",
    dest="speaker_path",
)


parser_speech2text = subparsers.add_parser(
    "speech2text", help="export model for speech2text endpoint"
)
add_common_arguments(parser_speech2text)
parser_speech2text.add_argument(
    "--num_streams",
    default=0,
    type=int,
    help="The number of parallel execution streams to use for the models in the pipeline.",
...
parser_speech2text.add_argument(
    ...
    default=False,
    action="store_true",
    help="Load model with word timestamps support.",
    dest="enable_word_timestamps",
)
        ...
        if "mediapipe_config_list" in config_data:
            for mp_config in config_data["mediapipe_config_list"]:
                if "name" in mp_config and "base_path" in mp_config:
                    if not any(
                        d["config"]["name"] == mp_config["name"] + "_model"
                        for d in config_data["model_config_list"]
                    ):
                        config_data["model_config_list"].append(
                            {
                                "config": {
                                    "name": mp_config["name"] + "_model",
                                    "base_path": mp_config["base_path"],
                                }
                            }
                        )
...
def export_text_generation_model(
    ...
    if (
        task_parameters.get("tool_parser", None) == "gptoss"
        or task_parameters.get("reasoning_parser", None) == "gptoss"
    ):
        if task_parameters.get("tool_parser", None) != task_parameters.get(
        ...
        if not os.path.isdir(llm_model_path) or args["overwrite_models"]:
            if task_parameters["target_device"] == "NPU":
                if precision != "int4" and precision != "nf4":
                    print(
                        "NPU target device requires int4 or nf4 precision. Changing to int4"
                    )
                    precision = "int4"
            ...
            if not (
                os.path.isfile(os.path.join(llm_model_path, "openvino_detokenizer.xml"))
            ):
                print(
    ...
    gtemplate = jinja2.Environment(loader=jinja2.BaseLoader).from_string(
        text_generation_graph_template
    )
    print("task_parameters", task_parameters)
    graph_content = gtemplate.render(
        model_path=model_path,
        draft_model_dir_name=draft_model_dir_name,
        **task_parameters,
    )
...
def export_embeddings_model_ov(
    model_repository_path,
    source_model,
    model_name,
    precision,
    ...
    if not os.path.isdir(destination_path) or args["overwrite_models"]:
        optimum_command = "optimum-cli export openvino --model {} --disable-convert-tokenizer --task feature-extraction --weight-format {} {} --trust-remote-code {}".format(
            source_model,
            precision,
            task_parameters["extra_quantization_params"],
            destination_path,
...
def export_text2speech_model(
    ...
    if not os.path.isdir(destination_path) or args["overwrite_models"]:
        optimum_command = 'optimum-cli export openvino --model {} --weight-format {} --trust-remote-code --model-kwargs "{{\\"vocoder\\": \\"{}\\"}}" {}'.format(
            source_model, precision, task_parameters["vocoder"], destination_path
        )
        if os.system(optimum_command):
...
def export_rerank_model(
    ...
    if os.path.isfile(os.path.join(model_name, "openvino_model.xml")):
        ...
        shutil.move(
            os.path.join(model_repository_path, model_name, "openvino_tokenizer.xml"),
            os.path.join(
                model_repository_path, model_name, "tokenizer", version, "model.xml"
            ),
        )
        shutil.move(
            os.path.join(model_repository_path, model_name, "openvino_tokenizer.bin"),
            os.path.join(
                model_repository_path, model_name, "tokenizer", version, "model.bin"
            ),
        )
        shutil.move(
            os.path.join(model_repository_path, model_name, "openvino_model.xml"),
            os.path.join(
                model_repository_path, model_name, "rerank", version, "model.xml"
            ),
        )
        shutil.move(
            os.path.join(model_repository_path, model_name, "openvino_model.bin"),
            os.path.join(
    ...
    stemplate = jinja2.Environment(loader=jinja2.BaseLoader).from_string(
        rerank_subconfig_template
    )
...
def export_image_generation_model(
    ...
    if os.path.isfile(model_index_path):
        ...
        optimum_command = (
            "optimum-cli export openvino --model {} --weight-format {} {} {}".format(
                source_model,
                precision,
                task_parameters["extra_quantization_params"],
                target_path,
    ...
    gtemplate = jinja2.Environment(loader=jinja2.BaseLoader).from_string(
        image_generation_graph_template
    )
